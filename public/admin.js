  ...
  api('/api/admin/tax-envelope').then((envelope) => {
    tabsNode.innerHTML = `<h1>Taxes</h1>
      ...
      <div class="row" style="margin-top:.5rem">
        <a class="btn" href="https://www.irs.gov/payments/direct-pay" target="_blank" rel="noopener noreferrer">Pay IRS (Direct Pay)</a>
        <button id="openPaymentSummary" type="button">Printable Payment Summary</button>
      </div>
    ...
    document.getElementById('openPaymentSummary').onclick = ()=> window.open('/api/admin/tax-envelope/payment-summary','_blank');
...
function renderGiving(){
  api('/api/admin/giving-envelope').then((giving) => {
    ...
    tabsNode.innerHTML = `<h1>Tithe & Offering Envelope</h1>
      <p>Move money from Pure Sole into dedicated giving envelopes so it is set aside and trackable.</p>
      <div class="grid">
        ${card('Tithe Balance', `<div class="kpi">${m(giving.titheBalance)}</div>`)}
        ${card('Offering Balance', `<div class="kpi">${m(giving.offeringBalance)}</div>`)}
        ${card('Total Giving Balance', `<div class="kpi">${m(giving.totalGivingBalance)}</div>`)}
        ${card('Suggested Tithe (10% of profit)', `<div class="kpi">${m(giving.suggestedTithe)}</div>`)}
      </div>
      <div class="card">
        <h3>Add to Envelope</h3>
        <form id="givingDepositForm" class="row">
          <select class="input" name="bucket"><option value="tithe">Tithe</option><option value="offering">Offering</option></select>
          <input class="input" name="amount" type="number" step="0.01" min="0.01" placeholder="Amount" />
          <select class="input" name="method">${methodOptions}</select>
          <input class="input" name="note" placeholder="Note (optional)" />
          <button>Deposit</button>
        </form>
      </div>
      <div class="card" style="margin-top:.75rem">
        <h3>Send/Payout from Envelope</h3>
        <form id="givingWithdrawForm" class="row">
          <select class="input" name="bucket"><option value="tithe">Tithe</option><option value="offering">Offering</option></select>
          <input class="input" name="amount" type="number" step="0.01" min="0.01" placeholder="Amount" />
          <select class="input" name="method">${methodOptions}</select>
          <input class="input" name="note" placeholder="Note (e.g. church transfer)" />
          <button>Withdraw</button>
        </form>
      </div>
      <h3>Giving Transactions</h3>
      <table class="table"><tr><th>Time</th><th>Type</th><th>Bucket</th><th>Amount</th><th>Method</th><th>Note</th></tr>
        ${(giving.transactions || []).map(t=>`<tr><td>${new Date(t.createdAt).toLocaleString()}</td><td>${t.type}</td><td>${t.bucket}</td><td>${m(t.amount)}</td><td>${t.method}</td><td>${t.note || ''}</td></tr>`).join('') || '<tr><td colspan=\"6\">No giving activity yet</td></tr>'}
      </table>`;

    document.getElementById('givingDepositForm').onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      await api('/api/admin/giving-envelope/deposit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    document.getElementById('givingWithdrawForm').onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      await api('/api/admin/giving-envelope/withdraw', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          bucket: fd.get('bucket'),
    ...
    tabsNode.innerHTML = `<h1>Tithe & Offering Envelope</h1><p>Could not load envelope: ${err.message}</p>`;
  });
}
...
function renderSettings(){
  ...
  tabsNode.innerHTML = `<h1>Settings</h1>
    <form id="settingsForm" class="card">
      ...
      <h3>Payment Method Catalog (one per line)</h3>
      <textarea class="input" name="paymentMethodCatalog" rows="6">${(s.paymentMethodCatalog || []).join('\n')}</textarea>
      <h3>API Keys</h3>
...
async function saveSettings(e){
  e.preventDefault();
  const fd = new FormData(e.target);
  const paymentMethodCatalog = String(fd.get('paymentMethodCatalog') || '')
    .split('\\n')
    .map((x) => x.trim())
    .filter(Boolean);
